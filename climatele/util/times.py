            ...
            for i in range(nslab):
                ...
                for it in numpy.sort(timesleft[i].keys()):
                    ...
                    if val.value == vals[v]:
                        ...
                        bnds[v][0] = switchCalendars(bnds[v][0], tim.units, tim.getCalendar(), times[0].units,
                                                     times[0].getCalendar()).value
                        bnds[v][1] = switchCalendars(bnds[v][1], tim.units, tim.getCalendar(), times[0].units,
                                                     times[0].getCalendar()).value
                        del (timesleft[i][it])
...
class TimeSlicer:
    ...
    def departures(self, slab, slicerarg=None, criteriaarg=None, ref=None, statusbar=None, sum=False, detrend_window=0 ):
        """
...
class Seasons(ASeason):
    ...
    def departures(self, slab, slicerarg=None, criteriaarg=None, ref=None, statusbar=None, sum=False, detrend_window = 0):
        ...
        for i in range(n):
            ...
            out = TimeSlicer.departures(self, slab, slicerarg=self.departures_seasons[i], criteriaarg=criteriaarg,
                                        statusbar=statusbar, ref=newref, sum=sum, detrend_window=detrend_window)
            # Adds it to the list
            if out is not None:
                s.append(out)
        self.seasons = self.departures_seasons
